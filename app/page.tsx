  ...
  return <main>
    ...
    <section className="section category-section" id="collection">
      ...
      <div className="category-grid">
        <CategoryCard href={CP.bags} title="Leather & Suede Handbags" copy="Close-up craft, useful silhouettes and tactile materials." image={`${A}/lifestyle/look-scarf-on-bag.webp`}/>
        <CategoryCard href={CP.diamond} title="Diamond Jewellery" copy="Modern lab-grown brilliance in clean, wearable settings." image={`${A}/drive/lab-1.webp`} video={`${A}/motion/diamond-category-clean.mp4`}/>
        <CategoryCard href={CP.gemstone} title="Gemstone Jewellery" copy="Natural colour and individual stones made for layering." image={`${A}/drive/gem-1.webp`} video={`${A}/motion/gemstone-showcase.mp4`}/>
        <CategoryCard href={CP.scarves} title="Cashmere & Merino Wool Scarves" copy="Generous layers, soft fibres and an easy everyday drape." image={`${A}/editorial/scarf-lifestyle.webp`}/>
      </div>
    </section>

    <section className="trust-and-proof" id="proof">
      <div className="trust-box" aria-label="Why FableRoom">
        <TrustStat icon="⌂" image={`${A}/editorial/hero-women-fashion-lifestyle.webp`} end={14000} suffix="+" label="Happy homes"/>
        <TrustStat icon="◇" image={`${A}/drive/lab-1.webp`} end={6} prefix="£" suffix="M+" label="Customer savings"/>
        <TrustStat icon="✦" image={`${A}/editorial/a-plus-jewellery.webp`} end={100} suffix="%" label="Responsibly sourced"/>
        <TrustStat icon="—" image={`${A}/editorial/scarf-lifestyle.webp`} end={0} label="Middlemen markups"/>
      </div>
      <div className="section material-wrap"><div className="material-head"><div><p className="eyebrow">THE PROOF IS IN THE DETAIL</p><h2>Reasons to believe,<br/>seen up close.</h2></div><p>Non-clickable material studies explain what is specific and valuable about each collection—without creating another discovery layer.</p></div>
        <div className="material-rail">
          ...
          <Material images={[`${A}/generated/diamond-precision-setting.webp`,`${A}/generated/diamond-precision-brilliance.webp`]} index="02" title="Precision-set brilliance" copy="Lab-grown diamonds aligned in clean settings so proportion and light remain the focus."/>
    ...
    <CategoryEdit id="bags" variant="panorama" index="01" kicker="PURE LEATHER & SUEDE" title="The Bag Edit" copy="Useful shapes and specialist craft, organised by the way you carry." category="Bags" subcategories={[["All Bags",CP.bags],["Totes",CP.bags],["Cross Body",CP.bags],["Clutches & Pochettes",CP.bags],["Duffel & Weekender Bags",CP.bags]]} image={`${A}/editorial/hero-women-fashion-lifestyle.webp`} products={products.filter(p=>p.category==="Bags")} wish={wish} onWish={toggleWish} onOpen={openProduct}/>
    <CategoryEdit id="diamonds" variant="reverse" index="02" kicker="MODERN BRILLIANCE" title="The Lab-Grown Diamond Edit" copy="Clean settings and modern scale, with the product type visible before the individual piece." category="Diamond Jewellery" subcategories={[["All Diamond Jewellery",CP.diamond],["Earrings",CP.diamond],["Pendants",CP.diamond],["Charms",CP.diamond],["Bracelets & Chains",CP.diamond]]} image={`${A}/drive/lab-1.webp`} video={`${A}/motion/diamond-showcase.mp4`} products={products.filter(p=>p.category==="Diamond Jewellery")} wish={wish} onWish={toggleWish} onOpen={openProduct}/>
    <CategoryEdit id="gemstones" variant="mosaic" index="03" kicker="RAW & NATURAL" title="The Gemstone Edit" copy="Individual colour, organic variation and pieces designed to layer rather than match perfectly." category="Gemstone Jewellery" subcategories={[["All Gemstone Jewellery",CP.gemstone],["Earrings",CP.gemstone],["Pendants",CP.gemstone],["Charms",CP.gemstone],["Bracelets & Chains",CP.gemstone]]} image={`${A}/lifestyle/look-statement-gemstone.webp`} products={products.filter(p=>p.category==="Gemstone Jewellery")} wish={wish} onWish={toggleWish} onOpen={openProduct}/>
    ...
    <section className="section looks looks-premium" id="shop-the-look"><div className="section-heading split"><div><p className="eyebrow">SHOP THE LOOK</p><h2>Ten ways to wear the edit.</h2></div><div><p>Search-led outfit ideas combining handbags, jewellery and scarves. No hotspots—each image opens the complete look.</p><a className="underlink" href={LOOKS_PATH}>View all 10 looks <Icon name="arrow"/></a></div></div><div className="look-collage premium-look-board">{looks.slice(0,5).map((look,i)=><a key={look.id} className={`look-tile look-${i+1}`} href={`${LOOKS_PATH}#${look.id}`}><img src={look.image} alt={look.alt}/><span><i>{String(i+1).padStart(2,"0")}</i><small>{look.searchTerm}</small><b>{look.title}</b><em>Open complete look</em></span></a>)}</div></section>
