...
export default function Home() {
  ...
  return <main>
    ...
    <section className="look section"><div className="look-copy"><p className="eyebrow">A NEW WAY TO LIVE WITH FABLEROOM</p><h2>Designed to be wanted. Made to be lived in.</h2><p>A tactile leather bag, an easy cashmere-merino scarf and real-stone jewellery—considered separately, effortless together.</p></div><div className="look-image"><img src={`${A}/editorial/hero-women-fashion-lifestyle.webp`} alt="FableRoom bag, scarf and jewellery styled for everyday wear"/><i className="look-glow g1"/><i className="look-glow g2"/><i className="look-glow g3"/><button className="hot h1" onClick={()=>setQuick(products[0])} aria-label="View Apollo Crescent Suede Bag">+<span>Apollo suede bag</span></button><button className="hot h2" onClick={()=>setQuick(products[6])} aria-label="View Turquoise and Citrine Pendant">+<span>Real-stone pendant</span></button><button className="hot h3" onClick={()=>setQuick(products[9])} aria-label="View Cashmere Merino Wrap">+<span>Cashmere-merino wrap</span></button></div></section>

    <section className="proof section" id="proof"><div className="proof-visual"><img src={`${A}/bags/apollo_02/01-08-2026-productshoot5356.webp`} alt="Full view of a tan suede FableRoom bag"/></div><div className="proof-copy"><p className="eyebrow">MATERIALS, MAKE, EXPERTISE</p><h2>Beauty you can feel.<br/>Quality you can verify.</h2><p>Each piece begins with authentic materials, then passes through clean specialist workshops and the hands of artisans who understand how it should wear, drape and last.</p><div className="proof-points"><article><b>01</b><h3>Pure leather</h3><p>Full-grain leather and tactile suede, selected for natural character and longevity.</p></article><article><b>02</b><h3>Real gemstones</h3><p>Turquoise, moonstone, citrine and labradorite, each with its own natural variation.</p></article><article><b>03</b><h3>Cashmere & merino</h3><p>Soft, breathable fibres woven for warmth, drape and daily wear.</p></article></div></div></section>

    <section className="story-bands section" id="story"><div className="story-title"><p className="eyebrow">FROM MATERIAL TO MAKING</p><h2>The proof is in every detail.</h2></div>
      <article><div><span>PURE LEATHER</span><h3>Specialist craft, cleanly finished.</h3><p>Artisans shape the grain, stitching, hardware and useful interiors in specialist workshops—creating quality you can see and feel.</p><button className="text-link" onClick={()=>goToWorld("Bags")}>Explore leather bags <Icon name="arrow"/></button></div><img src={`${A}/bags/smart_08/01-08-2026-productshoot5447.webp`} alt="Full view of a softly structured leather bag"/></article>
      <article className="reverse"><div><span>REAL GEMSTONES</span><h3>Natural colour, chosen by experts.</h3><p>Moonstone, turquoise, citrine and labradorite are selected for individual character, then set in considered demi-fine pieces.</p><button className="text-link" onClick={()=>goToWorld("Jewellery")}>Explore jewellery <Icon name="arrow"/></button></div><img src={`${A}/editorial/turquoise-citrine-necklace.webp`} alt="Real turquoise and citrine pendant"/></article>
      <article><div><span>CASHMERE & MERINO</span><h3>The layer you reach for daily.</h3><p>Soft, generous and warm without bulk—woven to live by the door, in the car or over your coat, every day.</p><button className="text-link" onClick={()=>goToWorld("Scarves")}>Explore scarves <Icon name="arrow"/></button></div><img src={`${A}/editorial/cashmere-scarves.webp`} alt="Cashmere and merino scarves"/></article>
    </section>
    ...
    <section className="section category-section" id="collection"><div className="section-heading centered"><p className="eyebrow">EXPLORE BY CATEGORY</p><h2>Bags, jewellery and scarves—the FableRoom way.</h2><p>Considered collections shaped by authentic materials, specialist craft and everyday use.</p></div>
      <div className="category-grid">
        <CategoryCard title="Demi-Fine Jewellery" copy="Real stones. Real craft. Everyday pieces with personality." image={`${A}/editorial/jewellery-lifestyle.webp`} buttonLabel="Explore jewellery" onClick={() => goToWorld("Jewellery")}/>
        <CategoryCard title="Leather Handbags" copy="Full-grain leather and suede, shaped and finished by specialist makers." image={`${A}/bags/apollo_02/01-08-2026-productshoot5356.webp`} buttonLabel="Explore bags" onClick={() => goToWorld("Bags")}/>
        <CategoryCard title="Cashmere & Merino Scarves" copy="Soft, generous wraps made to be reached for every day." image={`${A}/editorial/scarf-lifestyle.webp`} buttonLabel="Explore scarves" onClick={() => goToWorld("Scarves")}/>
      </div>
    </section>

    <section className="section edit" id="edit"><div className="edit-heading"><div><p className="eyebrow">CURATED FOR EVERY DAY</p><h2>Bags, jewellery and scarves. One FableRoom point of view.</h2><p>Explore each edit through its materials, specialist making and the way it fits into everyday life.</p></div></div>
      <nav className="world-nav" aria-label="Explore categories"><button onClick={()=>goToWorld("Bags")}>01 · Bag edit</button><button onClick={()=>goToWorld("Jewellery")}>02 · Jewellery edit</button><button onClick={()=>goToWorld("Scarves")}>03 · Scarf edit</button></nav>
    </section>

    <CategoryWorld category="Bags" index="01" title="The Bag Edit" kicker="PURE LEATHER & SUEDE" copy="Tactile, useful silhouettes shaped by grain, stitching and the details you reach for every day." image={`${A}/bags/apollo_02/01-08-2026-productshoot5356.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick}/>
    <CategoryWorld category="Jewellery" index="02" title="The Jewellery Edit" kicker="REAL GEMSTONES" copy="Natural colour, fine scale and quiet character—made to layer, gift and wear every day." image={`${A}/editorial/jewellery-lifestyle.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick}/>
    <CategoryWorld category="Scarves" index="03" title="The Scarf Edit" kicker="CASHMERE & MERINO" copy="Soft, generous wraps in useful shades—made for warmth without bulk and everyday wear, not only occasions." image={`${A}/editorial/scarf-lifestyle.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick}/>
    ...
    {quick&&<Overlay close={()=>setQuick(null)}><QuickView p={quick} onClose={()=>setQuick(null)} wished={wish.includes(quick.id)} onWish={()=>toggleWish(quick.id)}/></Overlay>}
    {cartOpen&&<Overlay close={()=>setCartOpen(false)}><CartDrawer cart={cart} setCart={setCart} close={()=>setCartOpen(false)}/></Overlay>}
...
function CategoryWorld({category,index,title,kicker,copy,image,wished,onWish,onQuick}:{category:Category;index:string;title:string;kicker:string;copy:string;image:string;wished:number[];onWish:(id:number)=>void;onQuick:(p:Product)=>void}) { const rail=useRef<HTMLDivElement>(null); const pieces=products.filter(p=>p.category===category); const visualCopy:Record<Category,string>={Bags:"Made for the rhythm of every day.",Jewellery:"Natural colour, quietly considered.",Scarves:"The layer you reach for daily."}; const scroll=(direction:number)=>rail.current?.scrollBy({left:direction*440,behavior:"smooth"}); return <section className={`product-world world-${category.toLowerCase()}`} id={`world-${category.toLowerCase()}`}><div className="world-head"><div><span>{index}</span><p className="eyebrow">{kicker}</p><h2>{title}</h2><p>{copy}</p></div><div className="rail-controls"><button onClick={()=>scroll(-1)} aria-label={`Scroll ${category} left`}><Icon name="arrow"/></button><button onClick={()=>scroll(1)} aria-label={`Scroll ${category} right`}><Icon name="arrow"/></button></div></div><div className="product-rail" ref={rail}><article className="world-visual"><img src={image} alt={`${title} collection`}/><div><small>THE {category.toUpperCase()} EDIT</small><b>{visualCopy[category]}</b></div></article>{pieces.map(p=><ProductCard key={p.id} product={p} wished={wished.includes(p.id)} onWish={()=>onWish(p.id)} onQuick={()=>onQuick(p)}/>)}</div></section> }
...
function QuickView({p,onClose,wished,onWish}:{p:Product;onClose:()=>void;wished:boolean;onWish:()=>void}) { const [idx,setIdx]=useState(0); return <aside className="quick-view"><button className="panel-close" onClick={onClose}><Icon name="close"/></button><div className="quick-gallery"><img src={p.images[idx]} alt={p.name}/><div>{p.images.map((im,i)=><button key={im} className={idx===i?"active":""} onClick={()=>setIdx(i)}><img src={im} alt=""/></button>)}</div></div><div className="quick-copy"><p className="eyebrow">{p.category}</p><h2>{p.name}</h2><p>{p.material}</p><p>{p.note}</p><div className="colour"><span>Colour: <b>{p.colour}</b></span><i style={{background:p.colour.toLowerCase()}}/></div><ul><li>Authentic materials, clearly identified</li><li>Direct from specialist makers</li><li>Material and construction details included</li><li>14-day easy returns</li></ul><div className="quick-actions"><button className={`button dark ${wished?"wished":""}`} onClick={onWish}>{wished?"Saved to wishlist":"Save to wishlist"}</button></div></div></aside> }
function CartDrawer({cart,setCart,close}:{cart:Record<number,number>;setCart:React.Dispatch<React.SetStateAction<Record<number,number>>>;close:()=>void}) { const lines=products.filter(p=>cart[p.id]); const update=(id:number,n:number)=>setCart(c=>{const x={...c};if(n<=0)delete x[id];else x[id]=n;return x}); return <aside className="cart-drawer"><div className="cart-head"><div><p className="eyebrow">YOUR EDIT</p><h2>Shopping bag</h2></div><button onClick={close}><Icon name="close"/></button></div>{lines.length?<><div className="cart-lines">{lines.map(p=><article key={p.id}><img src={p.images[0]} alt=""/><div><h3>{p.name}</h3><p>{p.colour}</p><div className="qty"><button onClick={()=>update(p.id,cart[p.id]-1)}><Icon name="minus"/></button><span>{cart[p.id]}</span><button onClick={()=>update(p.id,cart[p.id]+1)}><Icon name="plus"/></button></div></div><button className="remove" onClick={()=>update(p.id,0)}>Remove</button></article>)}</div><div className="cart-bottom"><small>Pricing and purchase details are available on each product page.</small><button className="button dark" onClick={()=>alert("Product pages are ready for commerce integration.")}>View product details</button></div></>:<div className="cart-empty"><Icon name="bag"/><h3>Your bag is waiting.</h3><p>Discover real-stone jewellery, tactile scarves and specialist-made leather.</p><button className="button dark" onClick={close}>Continue exploring</button></div>}</aside> }
