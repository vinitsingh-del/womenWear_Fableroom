...
import { A, bag, drive, products, type Category, type Product } from "./products-data";
...
export default function Home(){
  ...
  return <main>
    ...
    <BagEdit products={products.filter(p=>p.category==="Bags")} wish={wish} onWish={toggleWish} onOpen={openProduct} onAdd={add}/>
    <CategoryEdit id="diamonds" variant="reverse" index="02" kicker="LAB-GROWN DIAMONDS" title="The Lab-Grown Diamond Edit" copy="Clean settings and modern scale, organised by jewellery type." category="Diamond Jewellery" subcategories={[["Necklaces",CP.diamond],["Bracelets",CP.diamond],["Earrings",CP.diamond],["Rings",CP.diamond]]} cutout image={`${A}/diamond/hero-earrings.webp`} tileImages={[`${A}/lifestyle2/dia-studs-green.webp`,`${A}/lifestyle2/dia-bracelet-rings.webp`,`${A}/lifestyle2/dia-huggies-suede.webp`,`${A}/lifestyle2/dia-rings-hand.webp`]} products={products.filter(p=>p.category==="Diamond Jewellery")}/>
    <CategoryEdit id="gemstones" variant="mosaic" index="03" kicker="RAW & NATURAL" title="The Gemstone Edit" copy="Individual colour, organic variation and pieces designed to layer." category="Gemstone Jewellery" subcategories={[["Earrings",CP.gemstone],["Pendants",CP.gemstone],["Cuffs",CP.gemstone],["Necklaces",CP.gemstone]]} image={`${A}/lifestyle2/gem-turquoise-pendant.webp`} tileImages={[`${A}/lifestyle2/dia-rings-gold.webp`,`${A}/lifestyle2/gem-turquoise-pendant.webp`,`${A}/lifestyle2/dia-rings-hand.webp`,`${A}/lifestyle2/dia-bracelet-rings.webp`]} products={products.filter(p=>p.category==="Gemstone Jewellery")}/>
    <ScarfEdit products={products.filter(p=>p.category==="Scarves")} wish={wish} onWish={toggleWish} onOpen={openProduct} onAdd={add}/>

    <section className="section trending" id="new"><div className="section-heading split"><div><p className="eyebrow">WHAT’S NEW & TRENDING</p><h2>A space for what comes next.</h2></div><p>Built to hold future drops, new arrivals and timely curated collections without rebuilding the page.</p></div><div className="trend-grid"><a className="trend-lead" href={CP.bags}><img src={`${A}/lifestyle/look-scarf-on-bag.webp`} alt="Scarf styled on a structured handbag"/><span><small>THE CURRENT NOTE</small><b>Scarf on the bag</b><em>Explore the collection</em></span></a><a href={CP.bags}><img src={`${A}/bags/apollo_02/01-08-2026-productshoot5356.webp`} alt="Tan suede shoulder bag"/><span><small>NEW MATERIAL</small><b>Suede, now</b><em>Explore the collection</em></span></a><a href={CP.scarves}><img src={`${A}/drive/scarf-1.webp`} alt="Merlot scarf styled on a model"/><span><small>COLOUR EDIT</small><b>The merlot layer</b><em>Explore the collection</em></span></a></div></section>
...
function ScarfEdit({products:items,wish,onWish,onOpen,onAdd}:{products:Product[];wish:number[];onWish:(id:number)=>void;onOpen:(p:Product)=>void;onAdd:(id:number)=>void}){
...
function BagEdit({products:items,wish,onWish,onOpen,onAdd}:{products:Product[];wish:number[];onWish:(id:number)=>void;onOpen:(p:Product)=>void;onAdd:(id:number)=>void}){
  const [filter,setFilter]=useState(0);
  const [active,setActive]=useState<number|null>(null);
  // Touch devices fire pointerenter while scrolling the rail, which swapped the
  // lifestyle banner for a product cutout. Only hover-capable pointers preview.
...
function ProductCard({product:p,wished,onWish,onOpen,onAdd,onPreview,onPreviewEnd}:{product:Product;wished:boolean;onWish:()=>void;onOpen:()=>void;onAdd:()=>void;onPreview?:()=>void;onPreviewEnd?:()=>void}){return <article onPointerEnter={onPreview} onPointerLeave={onPreviewEnd} onFocusCapture={onPreview} onBlurCapture={onPreviewEnd}><button className="scarf-image" onClick={onOpen}><img src={p.images[0]} alt={p.name} loading="lazy"/><span>View piece</span></button><div><h3>{p.name}</h3><small>{p.material}</small><b>£{p.price}.00</b><button className={`mini-wish ${wished?"wished":""}`} onClick={onWish} aria-label="Save to wishlist"><Icon name="heart"/></button><button className="atc-ic" onClick={onAdd} aria-label="Add to cart"><Icon name="bag"/></button></div></article>}
