...
const products: Product[] = [
  ...
  { id: 10, name: "Cashmere Merino Wrap", category: "Scarves", material: "Cashmere & fine merino · 70 × 200cm", price: 95, colour: "Oatmeal", images: [`${A}/editorial/cashmere-scarves.webp`, `${A}/editorial/scarf-lifestyle.webp`], note: "Generously sized to wrap twice, with a soft hand feel made for everyday wear." },
  { id: 11, name: "Everyday Merino Scarf", category: "Scarves", material: "Responsible merino wool · 70 × 200cm", price: 72, colour: "Camel", images: [`${A}/editorial/scarf-lifestyle.webp`, `${A}/editorial/cashmere-scarves.webp`], note: "Warm without bulk, woven for a fluid drape and finished with soft fringe." },
  { id: 12, name: "Charcoal Cashmere Blend", category: "Scarves", material: "Cashmere & merino · 70 × 200cm", price: 98, colour: "Charcoal", images: [`${A}/editorial/cashmere-scarves.webp`, `${A}/editorial/scarf-lifestyle.webp`], note: "A deep charcoal layer with an understated herringbone weave." },
];
...
export default function Home() {
  ...
  return <main>
    <div className="announce">Thoughtfully made, fairly priced <span/> Complimentary UK delivery on orders over £100</div>
    ...
    <section className="section category-section" id="collection"><div className="section-heading centered"><p className="eyebrow">SHOP BY CATEGORY</p><h2>Pieces for the way you live.</h2><p>Thoughtfully selected to wear, carry and keep.</p></div>
      <div className="category-grid">
        <CategoryCard title="Demi-Fine Jewellery" copy="Real stones. Real craft. Everyday pieces with personality." image={`${A}/editorial/jewellery-lifestyle.webp`} onClick={() => goToWorld("Jewellery")}/>
        <CategoryCard title="Leather Handbags" copy="Full-grain leather and suede, with the make—not the markup." image={`${A}/bags/apollo_02/01-08-2026-productshoot5356.webp`} onClick={() => goToWorld("Bags")}/>
        <CategoryCard title="Cashmere & Merino" copy="Soft, generous wraps made to be reached for every day." image={`${A}/editorial/scarf-lifestyle.webp`} onClick={() => goToWorld("Scarves")}/>
      </div>
    </section>

    <section className="section edit" id="edit"><div className="edit-heading"><div><p className="eyebrow">CURATED FOR EVERY DAY</p><h2>Three worlds. One FableRoom point of view.</h2><p>Move through the edit by material, mood and the way you wear it.</p></div><span>12 considered pieces</span></div>
    ...
    <CategoryWorld category="Bags" index="01" title="The Bag Room" kicker="LEATHER & SUEDE" copy="Tactile, useful silhouettes shaped by grain, stitching and the details you reach for every day." image={`${A}/bags/apollo_11/01-08-2026-productshoot5500.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick} onAdd={add}/>
    <CategoryWorld category="Jewellery" index="02" title="Jewellery Stories" kicker="REAL GEMSTONES" copy="Natural colour, fine scale and quiet character—made to layer, gift and wear without occasion." image={`${A}/editorial/jewellery-lifestyle.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick} onAdd={add}/>
    <CategoryWorld category="Scarves" index="03" title="The Soft Layers" kicker="CASHMERE & MERINO" copy="Generous wraps in useful shades, made for warmth without bulk and softness that lasts." image={`${A}/editorial/scarf-lifestyle.webp`} wished={wish} onWish={toggleWish} onQuick={setQuick} onAdd={add}/>

    <section className="look section"><div className="look-copy"><p className="eyebrow">SHOP THE LOOK</p><h2>Effortless, everyday, FableRoom.</h2><p>The crescent suede bag, a generous merino wrap and real-stone jewellery—considered separately, better together.</p><button className="button outline" onClick={()=>{[1,7,10].forEach(add); setCartOpen(true)}}>Add the look · £403</button></div><div className="look-image"><img src={`${A}/editorial/hero-women-fashion-lifestyle.webp`} alt="FableRoom fashion look"/><i className="look-glow g1"/><i className="look-glow g2"/><i className="look-glow g3"/><button className="hot h1" onClick={()=>setQuick(products[0])}>+<span>Apollo bag · £198</span></button><button className="hot h2" onClick={()=>setQuick(products[6])}>+<span>Gemstone pendant · £110</span></button><button className="hot h3" onClick={()=>setQuick(products[9])}>+<span>Cashmere wrap · £95</span></button></div></section>

    <section className="proof section" id="proof"><div className="proof-visual"><img src={`${A}/bags/apollo_11/01-08-2026-productshoot5500.webp`} alt="Close view of FableRoom leather bag craftsmanship"/></div><div className="proof-copy"><p className="eyebrow">THE MAKE, NOT THE MARKUP</p><h2>Luxury materials.<br/>A fairer way to price them.</h2><p>We use the same direct model that earned your trust in the home: work closely with specialist makers, choose materials carefully and skip inflated layers of retail markup.</p><div className="proof-points"><article><b>01</b><h3>Desire</h3><p>Full-grain leather, genuine gemstones and soft cashmere-merino.</p></article><article><b>02</b><h3>Proof</h3><p>Material details, maker stories and construction you can inspect closely.</p></article><article><b>03</b><h3>Price</h3><p>Direct from the workshops, considered without relying on reflex discounts.</p></article></div></div></section>
    ...
    <section className="newsletter"><p className="eyebrow light">THE FABLE ROOM EDIT</p><h2>New stones, new shades,<br/>first access.</h2><p>Sign up for new product drops, maker stories and considered offers.</p><form onSubmit={(e)=>{e.preventDefault();setToast("You’re on the list")}}><input type="email" required placeholder="Your email address" aria-label="Email address"/><button>Join the edit</button></form></section>
...
function CategoryCard({title,copy,image,onClick}:{title:string;copy:string;image:string;onClick:()=>void}) { return <article className="category-card"><img src={image} alt=""/><div><h3>{title}</h3><p>{copy}</p><button onClick={onClick}>Shop now <Icon name="arrow"/></button></div></article> }
